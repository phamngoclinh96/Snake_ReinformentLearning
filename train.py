...
def create_dqn_model(env, num_last_frames,weight=None):
    """
...
class Args:
    level = 'snakeai/levels/a.json'
    num_episodes = 10000

def main():
    parsed_args = Args()# parse_command_line_args(sys.argv[1:])

    env = create_snake_environment(parsed_args.level)
    model = create_dqn_model(env, num_last_frames=4,weight=None)
