...
class Field(object):
    ...
    def __init__(self, level_map=None,size = 30):
        """
    ...
    def create_level(self):
        ...
        try:
            map_cells = [[self._level_map_to_cell_type[symbol] for symbol in line]
                for line in self.level_map
            ]
            map_cells = [line+[CellType.WALL]*(self.width-len(line)) for line in map_cells]
            map_cells = map_cells + [[CellType.WALL]*self.width]*(self.height-len(map_cells))
            self._cells = np.array(map_cells)
